# Awordle Supabase Notes

## Current state

No Awordle Supabase project has been provisioned, linked, queried, or migrated. The repository contains the complete 41-file ordered migration history from the locked functional checkpoint. Those files are preserved byte-for-byte and must be applied only to a new, verified Awordle project under separate authorization.
...
1. Create and verify a dedicated Awordle Supabase project.
...
5. Configure Awordle-only Site URL and approved redirect URLs.
